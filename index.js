...
function temperatureInF(temp, format) {
    return (format === "F" ? temp : temperatureCtoF(temp)) + "F";//Like my use of the ternary operator? XP
}
...
function getCarInfoById(inventory, carID) {
    for(var i = 0; i < inventory.length; i++){
        var car = inventory[i];
        if(car.id === carID) return `This is a ${car.car_make} ${car.car_model}`;
    }
...
function getOlderCars(inventory, maxYear) {
    ...
    for(var i = 0; i < inventory.length; i++){
        var car = inventory[i];
        if(car.car_year <= maxYear) out.push(car);
    }
    return out;
}
...
function getGermanCars(inventory) {
    ...
    for(var i = 0; i < inventory.length; i++){
        var car = inventory[i];
        if(car.car_make === `Audi` || car.car_make === `Mercedes-Benz` || car.car_make === `Volkswagen` ||car.car_make === `BMW`) out.push(car);
    }
    return out;
}
